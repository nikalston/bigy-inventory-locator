(() => {
  const DB_NAME = 'bigy_locator';
  const DB_VERSION = 2;
  const STORE_NAME = 'products';
  const SETTINGS_STORE = 'settings';
  const LS_KEY = 'bigy_products';
    ...
    productList.innerHTML = filtered.map(p => `
      <div class="product-card" data-id="${p.id}">
        <div class="product-location">
          ...
          ${p.bay ? `<div class="bay-label">Sec ${esc(p.bay)}</div>` : ''}
        </div>
  ...
  async function importData(file) {
    try {
      ...
      alert('Failed to import. Make sure the file is a valid Big Y products JSON export.');
    }
  }
